...
% Allow LaTeX to place figures more flexibly and reduce blank space
...
\subsection{Problem Formulation: Stochastic FrozenLake as an MDP}
We formulate slippery FrozenLake as a finite episodic Markov Decision Process (MDP)
\(\mathcal{M}=(\mathcal{S}, \mathcal{A}, P, R, \gamma)\).
Each state \(s \in \mathcal{S}\) represents the agent's current grid position, including terminal hole and goal states.
The action space is \(\mathcal{A}=\{\text{Left}, \text{Down}, \text{Right}, \text{Up}\}\).
The transition function \(P(s' \mid s,a)\) is stochastic because the executed movement may deviate from the intended action.
The reward is sparse: the agent receives reward 1 only when it reaches the goal and 0 otherwise.
An episode terminates when the agent reaches the goal, falls into a hole, or exceeds the maximum step limit.
The learning objective is to find a policy that maximizes expected return, which we evaluate mainly by greedy success rate under stochastic transitions.

All reported experiments use \texttt{slippery=True}. In each step, the environment samples one of three outcomes: the intended direction, the left deviation, or the right deviation, each with probability $1/3$. This matches the standard FrozenLake stochastic transition idea and prevents the task from becoming deterministic maze following. Figure~\ref{fig:envoverview} illustrates the FrozenLake layout, terminal states, and the slippery transition setting used throughout the report.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\linewidth]{figures_compact/frozenlake_environment_overview.png}
    \caption{FrozenLake environment overview. The agent starts at \texttt{S}, reaches \texttt{G}, avoids holes \texttt{H}, and evaluates greedy policies under stochastic slippery transitions.}
    \label{fig:envoverview}
\end{figure}
...
\subsection{Key RL Methods and Evaluation Metrics}
...
The standard Gymnasium 8$\times$8 map is an important benchmark. It is widely used, reproducible, and challenging without being degenerate. In our run, its oracle success is 0.861, and the learned mean success across the seven algorithms is 0.709. Several algorithms learn strong policies on this map: Expected SARSA reaches 0.85, Q($\lambda$) reaches 0.85, optimistic Q-learning reaches 0.82, Q-learning reaches 0.78, SARSA($\lambda$) reaches 0.76, and SARSA reaches 0.74. Figure~\ref{fig:gym8standardcurve} shows the corresponding learning curves on this standard stochastic 8$\times$8 benchmark and confirms that the training pipeline can handle it.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.78\linewidth]{figures_compact/gymnasium8/standard8_learning_curve.png}
    \caption{Learning curves on the Gymnasium standard 8$\times$8 slippery map. This figure is kept as a standard reference point before the random-map diagnostic.}
    \label{fig:gym8standardcurve}
\end{figure}

Gymnasium-generated random maps answer a different design question: what happens if we rely directly on the library's random-map generator at lower frozen-tile probabilities? The answer is that performance drops sharply on many generated random maps, and the reason is visible from the oracle. Figure~\ref{fig:gym8success} summarizes the learned final success rates, while Figure~\ref{fig:gym8gap} shows the oracle-gap pattern behind them. Several $p=0.5$ and $p=0.6$ random maps have oracle success near zero under slippery dynamics. These maps are technically connected, because the generator guarantees a path from start to goal, but the stochastic transition model makes reliable completion extremely unlikely. Across all 13 Gymnasium maps, including the standard map and the random maps, optimistic Q-learning remains the best learned method, but its mean final success is only 0.235 because the near-impossible generated maps dominate the average. This diagnostic result explains why the project moved toward carefully controlled maps for fair algorithm ranking.
...
Figure~\ref{fig:designed4} reports the controlled random 4$\times$4 experiment. These maps also have exactly four holes and are therefore much more credible than high-$p$ random maps. In this aggregate, optimistic Q-learning has the highest mean success rate, but the result should still be read as a credibility check rather than a strong ranking claim because the map set is small and the cross-map variability is large. The controlled 4$\times$4 setting confirms that the algorithms behave plausibly on non-trivial small boards, while the more reliable algorithm ranking should come from the larger 8$\times$8 controlled benchmark.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.80_m0_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.80_m1_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.80_m2_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.80_m3_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.80_m4_learning_curve.png}
    \vspace{0.25em}

    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.85_m0_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.85_m1_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.85_m2_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.85_m3_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.85_m4_learning_curve.png}
    \vspace{0.25em}

    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.90_m0_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.90_m1_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.90_m2_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.90_m3_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.90_m4_learning_curve.png}
    \vspace{0.25em}

    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.95_m0_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.95_m1_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.95_m2_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.95_m3_learning_curve.png}\hfill
    \includegraphics[width=0.188\linewidth]{figures_compact/per_map_random8/p0.95_m4_learning_curve.png}
    \caption{Per-map learning curves for all 20 controlled random 8$\times$8 slippery maps. Each panel is inserted from the original per-map figure, preserving the map label, legend, and layout inset.}
    \label{fig:learningpanel}
\end{figure}

\subsection{Key Numerical Results}
Table~\ref{tab:keyresults} reports the learned-agent success rates across the main stochastic settings. This table is meant to be a compact ranking matrix rather than a cherry-picked list: every main tabular algorithm is shown in every reported setting where it was evaluated. The Gymnasium random column is a diagnostic average over the standard 8$\times$8 map plus the generated random maps, so it should not be mixed directly with the controlled benchmark columns.

\begin{table}[!htbp]
\centering
\caption{Mean final greedy evaluation success across stochastic settings. Higher is better.}
\label{tab:keyresults}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccccc}
\toprule
Method & Std. 4x4 & Ctrl. 4x4 & Fixed 8x8 & Gym Std. 8x8 & Gym Random Diag. & Ctrl. Random 8x8\\
\midrule
Q-learning & 0.770 & 0.464 & 0.760 & 0.780 & 0.134 & 0.879\\
SARSA & 0.723 & 0.444 & 0.453 & 0.740 & 0.113 & 0.895\\
Expected SARSA & 0.680 & 0.383 & 0.230 & 0.850 & 0.135 & 0.902\\
SARSA($\lambda$) & 0.430 & 0.371 & 0.513 & 0.760 & 0.143 & 0.783\\
Q($\lambda$) & 0.530 & 0.326 & 0.580 & 0.850 & 0.128 & 0.873\\
Optimistic Q & 0.767 & 0.661 & 0.817 & 0.820 & 0.235 & 0.988\\
Dyna-Q & 0.347 & 0.388 & 0.537 & 0.160 & 0.072 & 0.870\\
\bottomrule
\end{tabular}
}
\end{table}

The controlled random 8$\times$8 column is the main comparison result because it averages over 20 controlled stochastic maps with a longer 10,000-episode budget. The Gymnasium random diagnostic column is intentionally low: it includes several generated boards whose oracle success is near zero under slippery dynamics, which is why those boards are used to motivate controlled map design rather than to rank algorithms.
...
On fixed 8$\times$8 stochastic FrozenLake, the best Optuna validation scores were 0.830 for optimistic Q-learning, 0.795 for Dyna-Q, 0.735 for Q-learning, and 0.665 for Expected SARSA. Figure~\ref{fig:optuna} shows this comparison directly. Three-seed validation confirmed tuned optimistic Q-learning and tuned Dyna-Q as the strongest tuned candidates. Optuna is therefore a good choice for this report; Ray Tune would be more appropriate only if we scaled to many more maps or GPU-based neural RL.
...
\section{Discussion}
The first finding is methodological: map choice changes what the experiment measures. The Gymnasium standard 8$\times$8 board is useful because it is a recognizable stochastic benchmark and our agents learn reasonable policies on it. In contrast, several Gymnasium-generated low-$p$ boards have near-zero oracle success under slippery dynamics. These boards are connected, but slipping makes reliable completion unlikely. This is why the final comparison uses controlled boards rather than treating all generated maps as equally fair test cases.

The second finding is algorithmic: on controlled stochastic maps, exploration is the main bottleneck. Optimistic Q-learning performs best on the controlled random 8$\times$8 benchmark because optimistic initial values keep uncertain actions attractive until the agent discovers successful routes. Q-learning, SARSA, and Expected SARSA form a strong second group, but they depend more heavily on the exploration schedule. This also explains why longer training and per-map curves reduced the apparent variance seen in earlier aggregate plots.

The ablation results support this interpretation. The exploration ablation shows that optimistic initialization is more effective than UCB-Q or softmax exploration in this tabular FrozenLake setting. The oracle-gap and per-map plots then show where learned policies still fall short of the known-model oracle. Together, these analyses point to exploration and map-specific failure cases as the main practical issues, rather than suggesting that adding more complex backup variants automatically improves performance.

% \subsection{Open Research Challenges}

% The main limitation is scope. The controlled maps make algorithm comparison clearer, but they do not prove generalization to every possible FrozenLake layout. Gymnasium random maps are therefore kept as a diagnostic boundary case, and the controlled random 8$\times$8 benchmark is presented as the main fair comparison rather than a universal claim. We also leave 16$\times$16 and neural-RL variants as future work because preliminary runs were strongly affected by map design and training budget.

\subsection{Open Research Challenges}

Several open challenges remain beyond this coursework-scale benchmark.
First, controlled maps make algorithm comparison fairer, but they do not prove generalization to every possible FrozenLake layout.
A future benchmark should test whether agents remain robust across a much wider distribution of stochastic maps.
Second, sparse rewards and slippery transitions still create difficult exploration problems: even when the oracle success is high, model-free agents may fail to discover reliable paths within a limited training budget.
Third, scaling from tabular 4$\times$4 and 8$\times$8 boards to larger 16$\times$16 maps or neural RL agents raises new sample-efficiency and representation-learning challenges.
Finally, benchmark design itself remains an open issue: deterministic connectivity is not enough under stochastic dynamics, so future work should develop principled map-generation criteria that balance solvability, difficulty, and fairness.

\subsection{Practical Implications and Reproducibility}

This project also provides several practical lessons for running small-scale RL experiments. First, a strong reported score is meaningful only when the environment difficulty is clearly controlled. On FrozenLake, a map can be reachable in the deterministic sense but still be nearly unsolvable under slippery stochastic transitions. Therefore, reporting the map-generation rule, the frozen-tile probability, and the Value Iteration oracle is necessary for interpreting learned-agent performance. Without these details, a low score may incorrectly look like an algorithmic failure, while a high score may simply come from an easy board.

Second, the evaluation protocol should be separated from the training behavior. During training, agents may explore using epsilon-greedy actions, optimistic initialization, softmax exploration, or UCB-style bonuses. During evaluation, however, we freeze the learned value table and test the greedy policy under the same stochastic environment. This separation makes the success rate easier to interpret: it measures the quality of the final learned policy rather than the randomness of the training policy.

Third, averaging across maps is useful but can hide important failures. Our controlled random 8$\times$8 benchmark shows high mean performance for several agents, but the per-map curves and oracle-gap analysis reveal that some methods still fail on isolated layouts. This is why we report both aggregate results and per-map diagnostics. The aggregate score supports a compact ranking, while the per-map view explains whether the ranking is stable or dominated by a few special cases.

Finally, the project emphasizes reproducibility. The experiments use fixed map-generation procedures, repeated evaluation, saved figures, and a public code repository. These choices make the comparison easier to inspect and rerun. For future coursework or research projects, this is an important lesson: in RL, the training algorithm is only one part of the experiment. Environment design, evaluation protocol, random seeds, oracle baselines, and result visualization all affect whether the final conclusion is trustworthy.
...
% \appendix
% \section*{Appendix: Team Contribution Statement}
